...
$(function(){
	...
	$.extend(speedTest['prototype'], {
		...
		ping: function(name, host, res, fn){
    		...
    		img.src = 'http://' + host + '/images/login/login-logo.png?' + new Date().getTime();
		...
		finish: function(fn){
			...
			for (var i = 0; i < data.length; i++) {
				...
				if(type != 'load'){
					desc = '<span class="slow-t">该线路访问异常，请使用别的线路</span>';
					speedStyleName = 'slow';
				}

				html += ['<li>',
					'<p class="routh-name">线路名称：<a style="text-decoration: underline" href="http://' + host + '">' + name + '</a> ' + desc + ' </p>',
					'<p class="process">',
